#' @title Dumped Double Exponential Smoothing - Gardner-McKenzie-Holt's algorithm
#'
#' @author Athanasios Migdalas, \email{athmig59@gmail.com}
#'
#' @description 
#' Forecasting method of dumped double exponential smoothing for time series model with additive trend and no seasonality.
#' 
#' @seealso sma, wma, naive, ses, ma, tes, des, dtes, lrtp
#' 
...
#' @param{phi}    {dumping factor; default value of 1 results in classic DES, i.e, Holt's algorithm}
...
#' ddes(d=dTest1, future=5)
#' ## Run with other parameter values
#' ddes(d=dTest1, future=5, alpha=0.6, beta=0.5, phi=0.7)
#' ## Run with non default initialization values for level and trend
#' ddes(d=dTest1, future=5, alpha=0.6, beta=0.5, phi=0.7, a0=95, b0=85)
#' 
#' @export
ddes <- function(d, alpha=0.3, beta=0.2, phi=1, future=1, a0=0, b0=NaN){
  m <- length(d)
  ...
  f[1] <- a0 + b0
  a[1] <-  alpha * d[1] + (1-alpha) * ( a0 + phi * b0)
  b[1] <- beta * ( a[1] - a0) + (1-beta) * phi * b0
  f[2] <- a[1] + phi * b[1]
