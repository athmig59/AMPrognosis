#' @title Decomposition based on Symmetric Moving Average estimation of the Trend -- Multiplicative Model
#'
...
#'
#' @details
#' Requires AMProgosis::seasonal_mult(), AMPrognosis::sma()
#'
#' @seealso decomposition_lrtp_mult, decomposition_lrtp, decomposition_lrtp_add,
#' decomposition_sma_add, decomposition_sma, plot_decomposition
#'
#'
...
#' ##
#' res<-decomposition_sma_mult(dTest6, fp=4)
#' ##
...
#'
#' @export
decomposition_sma_mult <- function(d, fp=NaN){
  ...
  if( !is.na(fp) ) {
    ...
    s <- AMPrognosis::seasonal_mult(dd,fp)  # Seasonal components
