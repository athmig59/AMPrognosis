#' @title Dumped Triple Exponential Additive Smoothing - Gardner-McKenzie-Holt-Winters algorithm
#'
#' @author Athanasios Migdalas, \email{athmig59@gmail.com}
#'
#' @description 
#' Forecasting method of triple exponential smoothing for time series model with 
#' additive trend and seasonality. A Gardner-McKenzie dumping factor
#' is used to dump future trend
#' 
#' @details 
#' Requires AMPrognosis::seasonal_add()
#' 
#' @seealso sma, wma, naive, ses, ma, tes, ddes, dtes, lrtp, tes_add, tes_mult, dtes_mult
#' 
...
#' dtes_add(d=dTest6,fp=4)
#' ## Run for 5 future periods with dumping factor 0.8
#' dtes_add(d=dTest6, fp=4, future=5, phi=0.8)
#' ## Run with other parameter values
#' dtes_add(d=dTest6, fp=4, future=5, alpha=0.6, beta=0.5, gamma=0.2, phi=0.8)
#' ## Run with non default initialization values for level and trend
#' dtes_add(d=dTest6, fp=4, alpha=0.6, beta=0.5, gamma=0.2, phi=0.8, future=5, a0=400, b0=110)
#' 
#' @export
dtes_add <- function(d, fp=NaN, alpha=0.3, beta=0.2, gamma=0.1, phi=1, future=1, a0=NaN, b0=NaN){
  ...
  s <- AMPrognosis::seasonal_add(d,fp)
