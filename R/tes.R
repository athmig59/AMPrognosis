#' @title Triple Exponential Smoothing - Holt-Winters algorithm
#'
...
#' 
#' @details 
#' Requires AMPrognosis::tes_add(), AMPrognosis::tes_add(),
#' AMPrognosis::seasonal_mult(), AMPrognosis::seasonal_add()
#' 
#' @seealso sma, wma, naive, ses, ma, ddes, dtes, lrtp, tes_add, tes_mult, 
#' dtes_add, dtes_mult
#' 
...
#' tes(d=dTest6,fp=4, model="multiplicative")
#' 
#' @export
tes <- function(d, fp=NaN, alpha=0.3, beta=0.2, gamma=0.1, future=1, model=c("additive","multiplicative"), a0=0, b0=NaN){
  if ( model == "additive"){
    AMPrognosis::tes_add(d, fp, alpha, beta, gamma, future, a0, b0)
  } else if(model == "multiplicative"){
    AMPrognosis::tes_mult(d, fp, alpha, beta, gamma, future, a0, b0)
  } else{
    message("*** Error: tes does not provide for such a model ***")
    return(NULL)
  }
}