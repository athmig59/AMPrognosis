#' @title Triple Exponential Additive Smoothing - Holt-Winters algorithm
#'
...
#' 
#' @seealso sma, wma, naive, ses, ma, tes, ddes, dtes, lrtp, tes_mult, dtes_add, dtes_mult
#' 
...
#' tes_add(d=dTest6,fp=4)
#' ## Run for 5 future periods
#' tes_add(d=dTest6, fp=4, future=5)
#' ## Run with other parameter values
#' tes_add(d=dTest6, fp=4, future=5, alpha=0.6, beta=0.5, gamma=0.2)
#' ## Run with non default initialization values for level and trend
#' tes_add(d=dTest6, fp=4, alpha=0.6, beta=0.5, gamma=0.2, future=5, a0=400, b0=110)
#' 
#' @export
tes_add <- function(d, fp=NaN, alpha=0.3, beta=0.2, gamma=0.1, future=1, a0=NaN, b0=NaN){
  ...
  for(t in 2:fp){
    f[t] = a[t-1] + b[t-1] + s[t]
    a[t] = alpha * ( d[t] - s[t] ) + ( 1 - alpha ) * ( a[t-1] + b[t-1] )
    b[t] = beta * ( a[t] - a[t-1]) + ( 1- beta) * b[t-1]
  }
  
  if((fp+1) <= m){
    for (t in (fp+1):m) {
      f[t] = a[t-1] + b[t-1] + s[t-fp]
      a[t] = alpha * ( d[t] - s[t-fp] ) + ( 1 - alpha ) * ( a[t-1] + b[t-1])
      b[t] = beta * ( a[t] - a[t-1] ) + ( 1 - beta ) * b[t-1]
